...
if [ ! $2 ]; then
    ...
    PREVIOUS_TASK="--previous_task_model_path ./checkpoints/LLaVA/$BASE_NAME/DVQA_llava_lora"
fi
...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port 29600 ETrain/Train/LLaVA/train_mem.py \
    ...
    --data_path /data2/rundongwang/data/tmp/OpenCodeInterpreter/data/train.json \
    --image_folder /data2/rundongwang/data/tmp/OpenCodeInterpreter/data \
    ...
    --bf16 True \
    --output_dir ./checkpoints/LLaVA/$BASE_NAME/CodeInterpreter_llava_lora \
    --num_train_epochs 1 \
