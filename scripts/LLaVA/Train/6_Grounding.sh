...
if [ ! $2 ]; then
    ...
    PREVIOUS_TASK="--previous_task_model_path ./checkpoints/LLaVA/$BASE_NAME/VizWiz_llava_lora"
fi
...
################## LLaMA-2 ##################
    ...
    # --previous_task_model_path ./checkpoints/LLaVA/$BASE_NAME/VizWiz_llava_lora \
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port 29600 ETrain/Train/LLaVA/train_mem.py \
    ...
    --bf16 True \
    --output_dir ./checkpoints/LLaVA/$BASE_NAME/Grounding_llava_lora_lr2e-5_bs128 \
    --num_train_epochs 1 \
