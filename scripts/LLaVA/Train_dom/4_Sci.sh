...
if [ ! $2 ]; then
    ...
    PREVIOUS_TASK="--previous_task_model_path ./checkpoints/LLaVA/$BASE_NAME/DriveLM_llava_lora"
fi
if [ ! $3 ]; then
    EXPERT=""
else
    EXPERT="--expert_num $3"
    OUTPUT_DIR="./checkpoints/LLaVA/$BASE_NAME/Sci_llava_lora_MOE"
    PREVIOUS_TASK="--previous_task_model_path ./checkpoints/LLaVA/$BASE_NAME/DriveLM_llava_lora_MOE"
fi
################## LLaMA-2 ##################
