...
def eval_single(test_file, result_file):
    ...
    for result in results:
        ...
        bbox_string = re.search(r'<box>\[\[(.*?)\]\]</box>', grounding_gt['answer_bbox']).group(1)
        bbox_groundtruth = [float(x) for x in bbox_string.split(',')]
        size = grounding_gt['size']

        pred_bbox_match = re.search(r'<box>\[\[(.*?)\]\]</box>', result['text'])
        if not pred_bbox_match:
            continue

        pred_bbox = pred_bbox_match.group(1)
        try:
            bbox_pred = [float(x) for x in pred_bbox.split(',')]
            if len(bbox_pred) != 4:
                continue
        except:
            continue
