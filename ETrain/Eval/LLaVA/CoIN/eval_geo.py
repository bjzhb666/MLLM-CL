...
def parse_yes_no_question(ans, gt):
    if (ans.lower().startswith('yes') and gt.lower().startswith('yes')) or (gt.lower().startswith('no') and ans.lower().startswith('no')):
        return True
    return False
...
def eval_single(annotation_file, result_file):
    ...
    for result in results:
        ...
        # score = bertscore.compute(predictions=annotation, references=ground_truth, lang='en')['f1']
        score = 0
        if 'Unanswerable' in result['text'] :
            continue
        
        if parse_yes_no_question(result['text'], ground_truth):
            right += 1
            score = 1
