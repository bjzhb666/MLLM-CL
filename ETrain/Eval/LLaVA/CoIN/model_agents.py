...
class CustomDataset(Dataset):
    def __init__(self, args, questions, image_folder, tokenizer, image_processor, model_config):
        # self.qf = args.question_file.split('/')[-2]
        self.qf = args.qf
        self.questions = questions
        self.image_folder = image_folder
        print(args.result_folders)
        self.result_folders = args.result_folders
        self.tokenizer = tokenizer
        self.image_processor = image_processor
        self.model_config = model_config
        self.names = "GQA Grounding ScienceQA TextVQA VizWiz VQAv2".split(' ')
        # self.names = ["sqa", "textqa", "ImageNet", "vizwiz", "vqav2"]
        self.par = [f"{name}_{self.qf}" for name in self.names]
        # self.names = ["sqa", "textqa", "ImageNet", "gqa", "vizwiz", "grounding", "vqav2"]
        # self.ans = {name: [] for name in self.names}
        self.ad = []
