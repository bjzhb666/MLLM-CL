...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def get_image_features(
        ...
        attention_mask = torch.ones(
            input_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device
        )
        attention_mask = _expand_mask(attention_mask, input_embeds.dtype).to(
            input_embeds.device
        )  # [bsz, 1, tgt_seq_len, src_seq_len]
    ...
    def encode_image(self, image, mode):
        if mode == "InternVL-C":
            vision_outputs = self.vision_model(
                pixel_values=image, output_hidden_states=False, return_dict=True
            )
            image_embeds = vision_outputs[0]
            ...
            # ensemble
            backbone_embeds = backbone_embeds / backbone_embeds.norm(
                dim=1, keepdim=True
            )
    ...
    def encode_text(self, text):
        ...
        text_embeds = text_embeds[
            torch.arange(text_embeds.shape[0]), attention_mask.sum(1) - 1
        ]
