...
class LlamaCrossAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.k_proj = nn.Linear(
            self.vision_hidden_size, self.num_heads * self.head_dim, bias=False
        )
        self.v_proj = nn.Linear(
            self.vision_hidden_size, self.num_heads * self.head_dim, bias=False
        )
    ...
    def forward(
        ...
        key_states = (
            self.k_proj(vision_hidden_states)
            .view(bs_v, kv_len, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
        value_states = (
            self.v_proj(vision_hidden_states)
            .view(bs_v, kv_len, self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            and hidden_states.size(1) >= self.num_query_token
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        use_cross_attn = [
            idx % self.cross_attention_frequency == 0
            for idx in range(config.num_hidden_layers)
        ]
        self.layers = nn.ModuleList(
            [
                LlamaDecoderLayer(config, use_cross_attn[idx])
                for idx in range(config.num_hidden_layers)
            ]
        )
